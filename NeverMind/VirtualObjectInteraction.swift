...
//  NeverMind
//
//  Created by Sarah Yan on 1/5/19.
...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    // scene view to hit test against when moving virtual content.
    let sceneView: VirtualObjectARView
    
    var selectedObject: VirtualObject?
    
    // object that is tracked for use by the pan and rotation gestures.
    ...
    // tracked screen position used to update the `trackedObject`'s position in `updateObjectToCurrentTrackingPosition()`.
    ...
    func didRotate(_ gesture: UIRotationGestureRecognizer) {
         ...
         */
        trackedObject?.object_rotation -= Float(gesture.rotation)
